...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("Sales team booked webhook received:", JSON.stringify(body));
    ...
    if (existing && existing.length > 0) {
      ...
      const newTeamBooked = (row.sales_team_booked || 0) + increment;
      const newTotal = (row.self_booked || 0) + newTeamBooked;

      const { data, error } = await supabase
        .from("metrics")
        .update({
          sales_team_booked: newTeamBooked,
          appointments_booked: newTotal,
        })
        .eq("id", row.id)
        .select();
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          date: today,
          sales_team_booked: increment,
          appointments_booked: increment,
        })
        .select();
      if (error) throw error;
    ...
    console.log(`Sales team booked webhook: +${increment} for client ${resolvedClientId}`);

    return new Response(
      JSON.stringify({ success: true, metric: "sales_team_booked", incremented_by: increment, ...result }),
